import { Controller, Get, Post, Body, Delete, Param, Header } from '@nestjs/common';
...
export class ServiceRegistrationController {
    ...
    constructor(private serviceRegistation: ServiceRegistrationService) {}
    ...
    @Get("/getall") // TODO: WHY????????
    getServices() {
        return this.serviceRegistation.getAllServices();
    }
    
    /**
     * Receives requests to add a service to be registed to the database
     * 
     * @param service to be monitored
     */
    @Post()
    addService(@Body() service: IService) {
        return this.serviceRegistation.addService(service);
    }
    ...
    deleteSelection(@Param('id') id: string) {
        return this.serviceRegistation.deleteService(id);
    }

    @Post('/:id')
    retransmitServiceId(@Param('id') id: string) {
        return this.serviceRegistation.retransmitServiceId(id);
    }
}
